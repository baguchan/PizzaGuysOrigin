package baguchan.pizzaguysorigin.client;
...
@Mod.EventBusSubscriber(modid = PizzaGuysOrigin.MODID, value = Dist.CLIENT)
	...
	public static void renderEvent(RenderLivingEvent.Post<LivingEntity, EntityModel<LivingEntity>> event) {
		...
		entity.getCapability(PizzaGuysOrigin.SUPER_DASH_CAP).ifPresent(illusion -> {
			if (IPowerContainer.hasPower(entity, ModPowerTypes.SUPER_DASH.get())) {
				...
				renderer.getModel().renderToBuffer(posestack, vertexconsumer, light, i, 1.0F, 0.6F, 0.6F, (0.45F));
				posestack.popPose();
				posestack.pushPose();
				if (shouldSit && entity.getVehicle() instanceof LivingEntity) {
				...
				renderer.getModel().prepareMobModel(entity, f5, f8, partialtick);
				renderer.getModel().setupAnim(entity, f5, f8, f7, f2, f6);
				renderer.getModel().renderToBuffer(posestack, vertexconsumer, light, i, 0.6F, 1.0F, 0.6F, 0.15F);
